<script lang="ts">
import Vue from 'vue';
// https://github.com/RobinLinus/socialmedia-leak/blob/master/index.html
export default Vue.extend({
  name: 'SocialFingerprint',
  data () {    
    return {
      fingerPrintId: '',
      fp: {}
...
var leakSocialMediaAccounts = function() {
    var platforms = [{
    ...
    // },
     { // @TODO FIX IT
        domain: "https://twitter.com",
        redirect: "/login?redirect_after_login=%2Fsettings%2Fprofile",
        name: "Twitter"
    },
     { //OK
    ...
    platforms.forEach(function(network) {
        ...
        img.onload = function() {
          ...
          console.log(network.name + "   " + true);
        };
        img.onerror = function() {
          //  callback(network, false);
          console.log(network.name + "    " + false);

        };
    });
};
leakSocialMediaAccounts();
...
<div>
   <img
  v-on:load="show_login_status('Google', true)"
  @error="show_login_status('GooglePlus', false)"
...
<img src="https://twitter.com/login?redirect_after_login=%2Fimages%2Fspinner.gif"
 v-on:load="show_login_status('Twitter', true)"
  @error="show_login_status('Twitter', false)" 
/>
